...
function saveToDatabase(){
        ...
        var superSmash = db.collection("superSmash")
        // price: price,
    superSmash.get().then(function(superSmash){
            addGame(superSmash);
            //gameOnPage(doc);
            })
        }
...
function addGame(superSmash) {
    ...
    // changing inner.HTMLs
        nameP.innerHTML = superSmash.data().name;
        consoleP.innerHTML = superSmash.data().console;
...
// depending on the game name user inserts, picture shows up beside it 
    // the last of us
        if (superSmash.data().name === "the last of us" || superSmash.data().name === "The Last of Us"){
    ...
    // halo 3: odst
         } else if (superSmash.data().name === "halo 3: odst" || superSmash.data().name === "Halo 3: ODST"){  // halo 3: odst
            var gamePicture = document.createElement("div");
            var pic = document.createElement("img");
    ...
    // super smash bros. ultimate
        } else if (superSmash.data().name === "super smash bros ultimate" || superSmash.data().name === "super smash bros. ultimate" || superSmash.data().name === "Super Smash Bros. Ultimate"){     // super smash bros ultimate
            var gamePicture = document.createElement("div");
            var pic = document.createElement("img");
    ...
    // call of duty: black ops 2
        } else if (superSmash.data().name === "call of duty black ops 2" || superSmash.data().name === "call of duty: black ops 2" || superSmash.data().name === "Call of Duty: Black Ops 2"){     // call of duty black ops 2
            var gamePicture = document.createElement("div");
            var pic = document.createElement("img");
    ...
    // minecraft
        } else if (superSmash.data().name === "minecraft" || superSmash.data().name === "Minecraft"){     // minecraft
            var gamePicture = document.createElement("div");
            var pic = document.createElement("img");
    ...
    var gameSection_id = superSmash.id
    gameSection.id = gameSection_id
            ...
            //document.getElementById(gameSection.id).remove()
                    ...
                    var question = confirm("Are you sure you want to trade this game for Super Smash Bros. Ultimate. There will be a $5 processing and handling fee for this transaction")
                ...
                if(question){
                    ...
                    db.collection('superSmash').doc(superSmash.id).delete();
 ...
 function loadGames(){
        ...
        var superSmash = db.collection("superSmash")
        superSmash.get().then(function(querySnapshot){
         querySnapshot.forEach(function(superSmash){
